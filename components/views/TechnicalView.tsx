...
export const TechnicalView: React.FC<TechnicalViewProps> = ({ data }) => {
  if (data.length === 0) {
    return <p className="text-slate-500 italic text-center mt-8">No se discutieron consideraciones técnicas.</p>;
  }
  ...
  return (
    <div className="space-y-4">
      {data.map((item, index) => (
        <Card key={index}>
          <h4 className="font-bold text-md text-slate-200">{item.topic}</h4>
          <p className="text-sm text-slate-300 mt-2">{item.details}</p>
          {item.open_questions.length > 0 && (
            <div className="mt-3 pt-3 border-t border-slate-700/50">
              <h5 className="font-semibold text-sm text-slate-400 mb-1">Preguntas Abiertas:</h5>
